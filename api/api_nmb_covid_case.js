...
count(CASE WHEN ([stayHome_start_date] is null or [stayHome_start_date] > CONVERT(date, getdate())) and ([returnToWork_date]  is null or [returnToWork_date] > CONVERT(date, DATEADD(HOUR, 7, getdate()))) and [status] != 'fatality' THEN 1 END) as [hospital],
count(CASE WHEN ([stayHome_start_date] is not null and [stayHome_start_date] <= CONVERT(date, getdate())) and ([returnToWork_date]  is null or [returnToWork_date] > CONVERT(date, DATEADD(HOUR, 7, getdate()))) and [status] != 'fatality' THEN 1 END) as [home],
